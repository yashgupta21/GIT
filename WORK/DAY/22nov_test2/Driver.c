...
typedef struct 
{
	u_int16_t offset;	/* FPGA BAR memory offset */
        u_int8_t *value;		/* address of data bytes to write/read in FPGA */
...
static long ioctl(struct file *file, unsigned int cmd, long unsigned int arg)
{	
	recVariable=kmalloc(sizeof(recStruct),GFP_KERNEL);
        switch(cmd)
	{
		...
		case IOCTL_RD_FPGA_REGISTER:
			...
			if(recVariable->size == 1)
			{
				pr_info("size 1 byte before"); 
				recVariable->value=(u_int8_t*)&newValue_8;
				//recVariable.offset=100;
				//recVariable.size=10;
				//pr_info("Value in kernel = %d\n",(uint8_t *)recVariable.value);
			      	
				//memcpy(recVariable.value,&newValue_8,sizeof(newValue_8));
                                pr_info("Size 1 Byte after");
				//pr_info("Value = %d ",*((u_int8_t*)recVariable.value));
                       ...
                       {
                       	recVariable->value=(u_int8_t*)&newValue_16;
				//memcpy(recVariable.value,&newValue_16, sizeof(newValue_16));
                       ...
                       {
                       	pr_info("size 4 byte before"); 
                       	recVariable->value=(u_int8_t*)&newValue_32;
